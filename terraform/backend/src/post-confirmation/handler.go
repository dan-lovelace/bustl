...
func handler(event events.CognitoEventUserPoolsPostConfirmation) (events.CognitoEventUserPoolsPostConfirmation, error) {
	fmt.Printf("PostConfirmation of user: %s\n", event.UserName)
	fmt.Printf("event %+v\n", event)
	...
	if umErr != nil {
		return events.CognitoEventUserPoolsPostConfirmation{}, umErr
	}
	...
	if len(users) < 1 {
		return events.CognitoEventUserPoolsPostConfirmation{}, errors.New("User doesn't exist")
	}
	...
	if res.StatusCode != 200 {
		return events.CognitoEventUserPoolsPostConfirmation{}, errors.New("User create failed")
	}
	...
	if resErr != nil {
		return events.CognitoEventUserPoolsPostConfirmation{}, resErr
	}

	if len(appsyncResponse.Errors) > 0 {
		fmt.Println("dataString", dataString)
		return events.CognitoEventUserPoolsPostConfirmation{}, errors.New("GraphQL error")
	}
