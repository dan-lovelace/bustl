...
func List(ctx context.Context, event types.LambdaEvent, dbq *pgx.Conn) types.LambdaResponse {
	...
	// filter by note_type_id if it exists
	if eventArgs.NoteTypeId != nil {
		query = query.Where("note_type.id = ?", eventArgs.NoteTypeId)

		// order by sort position for note-type-specific queries
		query = query.OrderBy("note.sort_position")
	}

	// archived filter
	// if eventArgs.Filter != nil && eventArgs.Filter.Archived != nil {
	// 	// use provided filter value
	// 	query = query.Where("note.archived = ?", eventArgs.Filter.Archived)
	// } else {
	// 	// default
